...
const defaultState = {
  ...
  staff: [
    { id: "admin", username: "admin", password: "AzamatPrint-Admin-2026!", role: "admin" },
    { id: "staff", username: "qyzmetker", password: "AzamatPrint-Qyzmetker-2026!", role: "staff" }
  ]
};
...
const routeIds = [...publicSectionIds, ...pageScreenIds];
...
function loadState() {
  ...
  try {
    ...
    const contacts = { ...defaultState.contacts };
    Object.entries(saved.contacts || {}).forEach(([key, value]) => {
      contacts[key] = String(value || "").trim() || defaultState.contacts[key];
    });
    return {
...
function normalizeStaff(savedStaff) {
  const staff = Array.isArray(savedStaff) ? savedStaff : structuredClone(defaultState.staff);
  return staff.map((person) => {
    if (person.username === "admin" && person.password === "admin123") {
      return { ...person, password: "AzamatPrint-Admin-2026!" };
    }
    if (person.username === "qyzmetker" && person.password === "print123") {
      return { ...person, password: "AzamatPrint-Qyzmetker-2026!" };
    }
    return person;
  });
}
...
function safeRun(label, callback) {
  try {
    ...
    console.error(`Azamat Print render error: ${label}`, error);
  }
}
...
function renderAll() {
  safeRun("services", renderServices);
  safeRun("categories", renderCategories);
  safeRun("prices", renderPrices);
  safeRun("reviews", renderReviews);
  safeRun("contacts", renderContacts);
  safeRun("dashboard", renderDashboard);
  safeRun("auth", updateAuthUi);
  showRoute(false);
}
...
function updateActivePublicSection() {
  const pageTarget = (window.location.hash || "").slice(1);
  if (pageScreenIds.includes(pageTarget)) {
    updateActiveLinks(pageTarget);
    return;
  }
...
function renderContacts() {
  ...
  const buttonRoot = qs("#contactButtons");
  const detailsRoot = qs("#contactDetails");
  if (buttonRoot) buttonRoot.innerHTML = buttons;
...
document.querySelectorAll('a[href^="#"]').forEach((link) => {
  link.addEventListener("click", (event) => {
    ...
    if (!routeIds.includes(targetId)) return;
...
["searchInput", "categoryFilter", "materialFilter", "sortSelect"].forEach((id) => {
  qs(`#${id}`)?.addEventListener("input", renderPrices);
});
...
qs("#loginForm")?.addEventListener("submit", (event) => {
  ...
  if (!user) {
    qs("#loginError").textContent = "Логин немесе құпиясөз дұрыс емес.";
    return;
  }
...
qs("#dashboard")?.addEventListener("submit", (event) => {
  event.preventDefault();
...
qs("#dashboard")?.addEventListener("click", (event) => {
...
qs("#dashboard")?.addEventListener("click", async (event) => {
...
window.addEventListener("scroll", () => {
  window.requestAnimationFrame(updateActivePublicSection);
}, { passive: true });
